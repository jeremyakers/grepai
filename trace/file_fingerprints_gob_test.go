...
func TestGOBSignatureSavePublishesOneCoherentSnapshot(t *testing.T) {
	...
	st := NewGOBSymbolStore(filepath.Join(t.TempDir(), "symbols.gob"))
	path := "same.go"
	if err := st.SaveFileWithSignature(ctx, path, "hash-old", "version-old", []Symbol{{Name: "Old", File: path, Line: 1}}, nil); err != nil {
		t.Fatal(err)
	}
	...
	go func() {
		writerDone <- st.saveFile(ctx, path, "hash-new", &newVersion, []Symbol{{Name: "New", File: path, Line: 2}}, nil, func() {
			if st.mu.TryRLock() {
				st.mu.RUnlock()
				locked <- false
			} else {

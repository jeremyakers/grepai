...
func TestReconcileDeletedDirectoryRejectsUnavailableProjectRoot(t *testing.T) {
	for _, setup := range []struct {
		...
		t.Run(setup.name, func(t *testing.T) {
			...
			err := reconcileDeletedDirectory(context.Background(), projectRoot, "src", &mockVectorStore{listDocumentsResult: []string{"src/old.go"}}, nil, func(event watcher.FileEvent) {

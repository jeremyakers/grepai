...
func handleFileEvent(ctx context.Context, idx *indexer.Indexer, scanner *indexer.Scanner, extractor *trace.RegexExtractor, symbolStore trace.SymbolStore, rpgEncoder *rpg.RPGEncoder, vectorStore store.VectorStore, enabledLanguages []string, projectRoot string, cfg *config.Config, lastConfigWrite *time.Time, rpgManager *rpgRealtimeManager, event watcher.FileEvent, onActivity watchActivityObserver, onStats watchStatsObserver, processors ...*framework.ProcessorRegistry) {
	forgetIndex := func(path string, eventType watcher.EventType) {
		...
		if symbolStore != nil {
			if err := symbolStore.DeleteFile(ctx, path); err != nil {
				log.Printf("Failed to remove symbols for %q: %v", path, err)
			}
		}
		if onStats != nil {
		...
		if rpgEncoder != nil {
			if err := rpgEncoder.HandleFileEvent(ctx, "delete", path, nil); err != nil {
				...
				log.Printf("rpg_event_applied_ms=%d file=%q event=%s rpg_dirty_files_count=%d", time.Since(start).Milliseconds(), path, eventType.String(), dirtyCount)
			}
		}
		log.Printf("Removed %q from index", path)
	}
	...
	case watcher.EventCreate, watcher.EventModify:
		...
		log.Printf("Indexed %q (%d chunks)", event.Path, chunks)
		...
		if isTracedLanguage(ext, enabledLanguages) {
			...
			if err != nil {
				...
				log.Printf("Extracted %d symbols from %q", len(symbols), event.Path)
				...
				if rpgEncoder != nil {
					...
					if rpgManager != nil {
						...
						dirtyCount, _, _, _ := rpgManager.Snapshot()
						log.Printf("rpg_event_applied_ms=%d file=%q event=%s rpg_dirty_files_count=%d",
